...
# Define edge colors corresponding to weights (normalized to 0-1 for Matplotlib)
COLORS = [
    (0, 1, 0),        # Soft Green
...
class MyGraph:
    def __init__(self):
        ...
        self.node_positions = {  # Predefined node positions for visualization
    ...
    def assign_weights_and_colors(self, min_weight:int=1, max_weight:int=5):
        """Assign random weights and colors to all edges."""
        for edge in self.graph.edges():
            weight = random.randint(min_weight, max_weight)
            self.graph[edge[0]][edge[1]]['weight'] = weight
            self.graph[edge[0]][edge[1]]['color'] = COLORS[weight - 1]
    ...
    def edge_weight(self, node1:str, node2:str):
        """Return the weight of the edge between two nodes."""
        return self.graph[node1][node2].get('weight') if self.graph.has_edge(node1, node2) else None
